...
subtitle: "Regression on Irish electricity demand aggregated by class"
...
We are analyzing a set of Irish household electricity demand available from the [`electBook` package](https://github.com/mfasiolo/electBook/tree/master). We have three datasets:
...
For ridge regression, we centered and scaled all variables used in the models to have mean 0 and standard deviation 1. For Gaussian process regression, we used the unscaled data.
...
The second class of models we implemented was Gaussian Process Regression.
...
We performed Gaussian process regression using the time counter variable generated in  [`feature_engineering.R`](https://github.com/Shermjj/SC-2-Electric-Boogalo/blob/main/feature_engineering.R) as the input variable $x$. As in ridge regression, we predicted demand for each of the five social classes at all three time scales (half hour, hour, and day).
...
We parallize over the grid of initial choices to speed up the optimisation. As we going to be performing this on different sized datasets (half hourly, hourly and daily) it is unwise to perform parallisation over those. Instead we created an wrapper function in `R` which can take in an arbitrary dataset. We display it below but it can be found in [`gauss_process_reg.R`](https://github.com/Shermjj/SC-2-Electric-Boogalo/blob/main/gauss_process_reg.R):
...
We can compare the MSE values for each class:

```{r}
day_gpr_mse <- readRDS(here::here("data/data_gpr/mse_results_day.RData"))
kableExtra::kable(day_gpr_mse,
                  col.names = c("Class", "Day: MSE"))
```
...
We can compare the hourly MSE values by class:

```{r}
hour_gpr_mse <- readRDS(here::here("data/data_gpr/mse_results_hour.RData"))
kableExtra::kable(hour_gpr_mse,
                  col.names = c("Class", "Hour: MSE"))
```
...
Finally, here are the half-hourly MSE values by class:

```{r}
halfhr_gpr_mse <- readRDS(here::here("data/data_gpr/mse_results_half_hour.RData"))
kableExtra::kable(halfhr_gpr_mse,
                  col.names = c("Class", "Half hour: MSE"))
```
...
We can compare the performance of the Gaussian process regression models between the social classes:

```{r}
overall_gpr_mse <- halfhr_gpr_mse %>% 
  left_join(hour_gpr_mse, by = "class") %>% 
  left_join(day_gpr_mse, by = "class")
colnames(overall_gpr_mse) <- c("class", "halfhr_MSE", "hr_MSE", "day_MSE")
...
overall_gpr_mse %>% 
  ...
  ggplot(aes(x = class, y = MSE, fill = time_scale)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Social class", y = "Gaussian process regression MSE", fill = "Time scale") +
  theme_bw()
```

The daily models have the best performance, with the hourly and half-hourly having similar, higher MSE values. For all time scales, the models on class DE have the worst performance, which is interesting given that DE contained the greatest number of households.

# Overall conclusions

In this project, we implemented ridge regression and Gaussian process regression to predict electricity demand for individual social classes at different time resolutions. 

We found that the ridge regression models performed relatively well on the test set at half-hourly and hourly resolutions, but their predictions did not reach to the daily maximum demands. The ridge regression models performed poorly at the daily resolution across all social classes, potentially indicating that we need to account for seasonality differently at this time resolution. The social class containing the lowest number of households, class F, had the poorest performance.

The Gaussian process regression models had reasonable fits on the training data at all time scales.  As with ridge regression, these models also failed to fully reach the daily maximum peaks, perhaps indicating that both our models are missing some key features causing the maximum peaks. Interestingly, Gaussian process regression was able to capture the patterns of daily fluctuations in the training data as well as ridge regression, despite not only using the time counter as a predictor and excluding temperature, Fourier terms, etc. The Gaussian process regression models had poor predictive ability on the test set for all resolutions, which may be due to the Gaussian kernel's tendency to trail off at the end of the data.

We cannot directly compare the performances of the two types of models using the MSE values since ridge regression was performed on scaled data and Gaussian process regression was performed on the unscaled. Additionally, the half-hourly and hourly Gaussian process regression models were only run on one week of the data due to computational constraints. When examining the overall patterns in MSE for each set of models, however, the ridge regression performed best on the half-hour and hour resolution, while Gaussian process regression was most successful at the day resolution. When comparing between social classes, the ridge regression models had poor performance on class F, the class with fewest households, while the Gaussian process regression actually performed worst on class DE, the class with the most households. 
