...
subtitle: "SC/SM-2 Group Project"
...
We trimmed the dataset slightly to focus on the year of 2010 only.
...
We also have the hyperparameter $\sigma$ which is the standard deviation of our noise. In order to find the ideal hyperparameters we will attempt to maximise the marginal log likelihood. The marginal log likelihood is given by:
...
In this project, we implemented two different approaches for modelling to predict electricity demand for individual social classes at different time resolutions. Firstly, a relatively simpler, parametric approach with a ridge regression model was used, with significant feature transformation. Secondly, a more flexible non-parametric Gaussian Process Regression model was used.   
