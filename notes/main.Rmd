...
**Insert example of `generate_fourier_terms` function**
...
We implemented ridge regression using `RcppArmadillo` in our function `RidgeReg`. We also wrote a function `RidgeRegPar` for ridge regression using `RcppParallel` to parallelise the matrix multiplication. We can compare the results of `RidgeRegPar` with those of the `glmnet` package:

```{r}
# library(glmnet)
# 
# # Example data
# ex_x <- matrix(rnorm(100 * 20), 100, 20)
# ex_y <- rnorm(100)
# 
# # Fit ridge regression using glmnet
# fit_glmnet <- cv.glmnet(ex_x,
#                         ex_y,
#                         family = "gaussian",
#                         alpha = 0,
#                         type.measure = "mse",
#                         nfolds = 10)
# ex_pred_glmnet <- predict(fit_glmnet, newx = ex_x)
# 
# # Fit ridge regression using RidgeRegPar
# fit_RidgeRegPar <- RidgeRegPar(ex_x,
#                                ex_y, 
#                                # Use same lambda as glmnet for comparison
#                                lambda = fit_glmnet$lambda.1se * nrow(ex_x)
#                                )
# ex_pred_RidgeRegPar <- cbind(1, ex_x) %*% fit_RidgeRegPar$coefficients
# 
# # Compare predictions
# ex_df <- data.frame(y = ex_y,
#                     glmnet_pred = ex_pred_glmnet,
#                     RidgeRegPar_pred = ex_pred_RidgeRegPar)
...
```{r}
as.data.frame(lapply(halfhr_ridge_preds, function(x) x$error)) %>% 
  pivot_longer(cols = everything(), names_to = "class", values_to = "MSE") %>% 
...
```{r}
as.data.frame(lapply(hr_ridge_preds, function(x) x$error)) %>% 
  pivot_longer(cols = everything(), names_to = "class", values_to = "MSE") %>% 
  # Remove _preds suffix from class column
  mutate(class = gsub("_preds", "", class))
```
...
```{r}
as.data.frame(lapply(day_ridge_preds, function(x) x$error)) %>% 
  pivot_longer(cols = everything(), names_to = "class", values_to = "MSE") %>% 
  # Remove _preds suffix from class column
  mutate(class = gsub("_preds", "", class))
```
...
We began by running our `Rcpp` code to find optimal hyperparameters for the Gaussian Process Regression model. We found that the optimal hyperparameters for the half-hourly, hourly, and daily datasets were as follows:

- Half-hourly: $l = 0.01, \sigma = 12062$
- Hourly: $l = 0.5, \sigma = 982$
- Daily: $l = 1, \sigma = 104$
...
# Load Day Data
df_day_AB <- readRDS(here::here("data/data_gpr/df_day_gpr_AB.RData"))
df_day_C1 <- readRDS(here::here("data/data_gpr/df_day_gpr_C1.RData"))
df_day_C2 <- readRDS(here::here("data/data_gpr/df_day_gpr_C2.RData"))
df_day_DE <- readRDS(here::here("data/data_gpr/df_day_gpr_DE.RData"))
df_day_F <- readRDS(here::here("data/data_gpr/df_day_gpr_F.RData"))

# Load Hour Data
df_hr_AB <- readRDS(here::here("data/data_gpr/df_hour_gpr_AB.RData"))
df_hr_C1 <- readRDS(here::here("data/data_gpr/df_hour_gpr_C1.RData"))
df_hr_C2 <- readRDS(here::here("data/data_gpr/df_hour_gpr_C2.RData"))
df_hr_DE <- readRDS(here::here("data/data_gpr/df_hour_gpr_DE.RData"))
...
For daily data, we had a very good fit with our estimated `sigma` and `l` values. 
...
Unfortunately, as a side affect of the gaussian kernel, we has considerable trail off at the end of the data. Meaning the `mse` on our test set is quite high. In terms of general fit for the training set it performs quite well.

### Aggregated Hourly Data

For hourly data, things are certainly a bit more messy, we theorize that the with more tuning of the hyperparameters we would get closer approximation of the daily cycles - which was not an issue with daily aggregated data. This is clearly an the outlier as we will see from the half-hourly data.

```{r}

image_files2 <- c(here::here("data/plots_gpr/hour__AB.jpeg"),
                  here::here("data/plots_gpr/hour__C1.jpeg"),
                  here::here("data/plots_gpr/hour__C2.jpeg"),
                  here::here("data/plots_gpr/hour__DE.jpeg"))
...
Unfortunately the amount of data would make it impossible to see any results, so we limit our view to the first week of the year, where we can more easily make out the models fit.
...
Unfortunately, despite considerble speed up of the optimisation, the `Rcpp` code did not provide us hyperparameters that were particularly useful from the hour and half-hourly data, however this may be due to the range and size of the data considering daily data was quite successful. 
