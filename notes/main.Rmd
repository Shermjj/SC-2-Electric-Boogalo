...
For hourly data, we can clearly see the daily cycles appearing, fortunately our GPR is able to get a pretty accurate fit, including the daily peaks of electricity consumption.

things are certainly a bit more messy, we theorize that the with more tuning of the hyperparameters we would get closer approximation of the daily cycles - which was not an issue with daily aggregated data. This is clearly an the outlier as we will see from the half-hourly data.
