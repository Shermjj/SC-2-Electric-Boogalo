...
halfhr_ridge_preds <- lapply(halfhr_ridge_results, function(cv_results) {
  preds <- predict_parallel_ridge_cv(model = cv_results,
                                     x_test = as.matrix(halfhr_features_test[,7:ncol(halfhr_features_test)]),
...
do.call(grid.arrange, c(halfhr_ridge_plots, ncol = 2))
```
...
# Get predictions for each model on the test set
hr_ridge_preds <- lapply(hr_ridge_results, function(cv_results) {
  preds <- predict_parallel_ridge_cv(model = cv_results,
                                     x_test = as.matrix(hr_features_test[,7:ncol(hr_features_test)]),
...
hr_test_true_preds <- hr_features_test %>% 
  select(dateTime, all_of(soc_classes)) %>% 
  # Add predictions
  bind_cols(as.data.frame(hr_ridge_preds))
...
do.call(grid.arrange, c(hr_ridge_plots, ncol = 2))
```
...
# Get predictions for each model on the test set
day_ridge_preds <- lapply(day_ridge_results, function(cv_results) {
  preds <- predict_parallel_ridge_cv(model = cv_results,
                                     x_test = as.matrix(day_features_test[,7:ncol(day_features_test)]),
...
day_test_true_preds <- day_features_test %>% 
  select(dateTime, all_of(soc_classes)) %>% 
  # Add predictions
  bind_cols(as.data.frame(day_ridge_preds))
...
do.call(grid.arrange, c(day_ridge_plots, ncol = 2))
```
