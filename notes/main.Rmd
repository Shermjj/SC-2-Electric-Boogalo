...
We implemented ridge regression using `RcppArmadillo`. To select $\lambda$, we used $k$-fold cross-validation, also implemented in `RcppArmadillo` using `RcppParallel` to parallelise the cross-validation process. These functions can be found in our package in the [`ridge_regression.h` file](https://github.com/Shermjj/SC-2-Electric-Boogalo/blob/main/ElecForecast/src/ridge_regression.h).
