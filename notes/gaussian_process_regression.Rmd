...
df_class1week <- filter(df_class1,week <= 2 & week > 1 )
#head(df_class1week)
...
sourceCpp("notes/gauss_process_reg.cpp")


x <- as.matrix(df_class1week_agg$dateTime)
y <- as.matrix(df_class1week_agg$demand)

xnew <- as.matrix(data.frame(time = seq(min(x), max(x), length.out = 1000)))
result <- gauss_process_regCpp(x,y,xnew,0.2,8)
