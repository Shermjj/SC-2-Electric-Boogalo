...
halfhr_features <- df_halfhr %>% 
  ...
  select(-dow)
# Add Fourier terms
fourier_agg_daily <- fourier(ts(halfhr_features$temp, frequency = 48), K = 5)
fourier_agg_yearly <- fourier(ts(halfhr_features$temp, frequency = 48 * 365), K = 5)
halfhr_features_fourier <- cbind(halfhr_features, fourier_agg_daily, fourier_agg_yearly)
...
# 80% training set
n_train <- round(0.8*nrow(halfhr_features_scaled))
