...
halfhr_features <- df_halfhr %>% 
  ...
  select(-dow)
# Add Fourier terms
fourier_halfhr_daily <- generate_fourier_terms(halfhr_features$counter, K = 5, period = 48)
...
fourier_halfhr_yearly <- generate_fourier_terms(halfhr_features$counter, K = 5, period = 48 * 365.25)
...
halfhr_features_fourier <- cbind(halfhr_features, fourier_halfhr_daily, fourier_halfhr_yearly) %>% 
  select(-counter)

# Scale input features
halfhr_features_scaled <- halfhr_features_fourier %>% 
  mutate(across(7:ncol(halfhr_features_fourier), scale))

# Split the data into training and test sets
# 90% training set
n_train <- round(0.9*nrow(halfhr_features_scaled))
halfhr_features_train <- halfhr_features_scaled[1:n_train,]
halfhr_features_test <- halfhr_features_scaled[-(1:n_train),]
```
...
List performRidgeRegression(const arma::mat& X, const arma::vec& y, double lambda) {
  ...
  double sig2 = arma::as_scalar(arma::trans(resid) * resid / n); // Use n instead of (n - k) to match the usual definition of MSE
